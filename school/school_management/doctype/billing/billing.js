...
function update_student_count(frm) {
    ...
    // Handle "Not Set" — students with no student_type or non-standard values
    if (frm.doc.status === 'Not Set') {
        filters['student_type'] = ['not in', ['Day', 'Boarding']];
    } else if (frm.doc.status) {
        filters['student_type'] = frm.doc.status;
    }
...
function toggle_student_filters(frm) {
    ...
    frm.set_df_property('status', 'hidden', single ? 1 : 0);
