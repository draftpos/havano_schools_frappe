...
class Student(Document):
	def validate(self):
		...
		if self.portal_password and self.student_reg_no and self.portal_password == self.student_reg_no:
			frappe.throw(
				"Portal Password cannot be the same as the Student Registration Number for security reasons."
			)
	...
	def _get_or_create_user(self, email, full_name, role):
		...
		if frappe.db.exists("User", email):
			...
			frappe.db.sql(
				"DELETE FROM `tabEmail Queue` WHERE reference_doctype='User' AND reference_name=%s", (email,)
			)
	...
	def create_parent_portal_users(self):
		...
		for entry in parent_entries:
			try:
				...
				user, is_new = self._get_or_create_user(entry["email"], entry["full_name"], entry["role"])
